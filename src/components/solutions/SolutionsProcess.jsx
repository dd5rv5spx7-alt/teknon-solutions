...
import { SOLUTIONS_PROCESS_STEPS } from '../../data/solutionsData.js';
      ...
      <div className="relative container-px mx-auto max-w-8xl grid lg:grid-cols-[minmax(0,1fr)_minmax(0,1.3fr)] gap-16">
        <div>
          <SectionHeading
            index="02"
            label="How it works"
            title="Our Engagement Process, Start to Finish"
            subtitle="Six steps take an engagement from first conversation to a retested, delivered result."
            light
          />
        </div>

        <div ref={ref}>
          {SOLUTIONS_PROCESS_STEPS.map((step, i) => (
            <StepRow
              key={step.step}
              step={step}
              isLast={i === SOLUTIONS_PROCESS_STEPS.length - 1}
              isInView={isInView}
