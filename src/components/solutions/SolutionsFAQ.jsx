...
import { SOLUTIONS_FAQS } from '../../data/solutionsData.js';

function FAQItem({ item, isOpen, onToggle, isInView, delay, itemId }) {
  const panelId = `solutions-faq-panel-${itemId}`;
  const buttonId = `solutions-faq-btn-${itemId}`;
  return (
      ...
      <div className="container-px mx-auto max-w-8xl">
        <div className="grid lg:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)] gap-14">
          <SectionHeading
            ...
            subtitle="Can't find what you're looking for? Send us your details below and we'll answer directly."
          />

          <div ref={ref}>
            {SOLUTIONS_FAQS.map((item, i) => (
              <FAQItem
                key={item.q}
                item={item}
                itemId={i}
