...
export default function StudentLogin() {
  const { signIn, isAuthenticated, loading, isSupabaseConfigured, role } = useAuth();
  ...
  // Gate on `loading`: isAuthenticated flips true before role has resolved,
  // so deciding the redirect target too early could send a staff account
  // to /student instead of /admin during that brief window.
  if (loading) {
    return (
  ...
  if (isAuthenticated) {
    // Staff accounts don't belong on the student portal — send them where they do belong.
    return <Navigate to={role === 'admin' || role === 'faculty' ? '/admin' : '/student'} replace />;
  }
  ...
  const handleSubmit = async (e) => {
    ...
    if (signInError) {
      ...
      navigate(signedInRole === 'admin' || signedInRole === 'faculty' ? '/admin' : '/student');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-grad-navy flex items-center justify-center px-6 relative overflow-hidden">
      <Seo
        title="Sign in — A Teknon Solutions"
        description="Sign in to your A Teknon Solutions student portal to view your courses, progress, and certificates."
        path="/student/login"
        noindex
      />
      ...
      <div className="relative w-full max-w-sm">
        ...
        <div className="glass rounded-3xl p-8">
          ...
          <p className="mt-1.5 text-sm text-white/55">
            Use the email and password from your account invite.
          </p>
          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-4">
            <div>
              <label htmlFor="student-email" className="block text-sm font-medium text-white/80 mb-1.5">
                Email
              </label>
              <input
                id="student-email"
            ...
            <div>
              <label htmlFor="student-password" className="block text-sm font-medium text-white/80 mb-1.5">
                Password
              </label>
              <PasswordField
                id="student-password"
          ...
          <p className="mt-5 text-xs text-white/55 text-center">
            No account yet? Contact A Teknon Solutions to get enrolled — student accounts aren&rsquo;t
            self-signup.
          </p>
        </div>
